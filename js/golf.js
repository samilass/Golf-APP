...
function getrandom() {
  ...
  $$.getJSON('http://golf-entertainment.com/recent_subscribers.php?callback=?',function(json){
...
myApp.onPageInit('autocomplete', function (page) {
    ...
    var autocompleteStandaloneAjax = myApp.autocomplete({
        ...
        source: function (autocomplete, query, render) {
            ...
            // Do Ajax request to Autocomplete data
            $$.ajax({
                url: 'http://golf-entertainment.com/dest.php?term='+query,
                method: 'GET',
                dataType: 'json',
                //send "query" to server. Useful in case you generate response dynamically
                data: {
                ...
                success: function (data) {
                    // Find matched items
                    for (var i = 0; i < data.length; i++) {
                        if (data[i].label.toLowerCase().indexOf(query.toLowerCase()) >= 0) results.push(data[i]);
                    }
                    // Hide Preoloader
                    autocomplete.hidePreloader();
                    // Render items by passing array with result items
